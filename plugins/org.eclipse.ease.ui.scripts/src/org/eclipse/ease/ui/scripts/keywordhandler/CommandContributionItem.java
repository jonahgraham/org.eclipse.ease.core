...
public class CommandContributionItem extends ContributionItem {
	...
	/**
	 * Mode bit: Show text on tool items or buttons, even if an image is present. If this mode bit is not set, text is only shown on tool items if there is no
	 * image present.
	 *
	 * @since 3.4
	 */
	...
	/**
	 * This is <code>true</code> when the menu contribution's visibleWhen checkEnabled attribute is <code>true</code>.
	 */
	 ...
	 * @since 3.4
	 */
	public CommandContributionItem(final CommandContributionItemParameter contributionParameters) {
		...
		serviceLocator = contributionParameters.serviceLocator;
		...
		visibleEnabled = contributionParameters.visibleEnabled;
		mode = contributionParameters.mode;

		menuService = (IMenuService) contributionParameters.serviceLocator.getService(IMenuService.class);
		commandService = (ICommandService) contributionParameters.serviceLocator.getService(ICommandService.class);
		handlerService = (IHandlerService) contributionParameters.serviceLocator.getService(IHandlerService.class);
		bindingService = (IBindingService) contributionParameters.serviceLocator.getService(IBindingService.class);
		IWorkbenchLocationService workbenchLocationService = (IWorkbenchLocationService) contributionParameters.serviceLocator
		...
		if (command != null) {
			...
			if (contributionParameters.helpContextId == null) {
				try {
					helpContextId = commandService.getHelpContextId(contributionParameters.commandId);
				} catch (NotDefinedException e) {
					// it's OK to not have a helpContextId
				}
			}
			IWorkbenchLocationService wls = (IWorkbenchLocationService) contributionParameters.serviceLocator.getService(IWorkbenchLocationService.class);
			final IWorkbench workbench = wls.getWorkbench();
			if ((workbench != null) && (helpContextId != null)) {
				workbenchHelpSystem = workbench.getHelpSystem();
			}
		}
	 ...
	 *            a service locator that is most appropriate for this contribution. Typically the local {@link IWorkbenchWindow} or {@link IWorkbenchPartSite}
	 *            will be sufficient.
	 * @param id
	 ...
	 * @param parameters
	 *            A map of strings to strings which represent parameter names to values. The parameter names must match those in the command definition.
	 * @param icon
	 ...
	 */
	@Deprecated
	public CommandContributionItem(final IServiceLocator serviceLocator, final String id, final String commandId, final Map parameters,
			final ImageDescriptor icon, final ImageDescriptor disabledIcon, final ImageDescriptor hoverIcon, final String label, final String mnemonic,
			final String tooltip, final int style) {
		this(new CommandContributionItemParameter(serviceLocator, id, commandId, parameters, icon, disabledIcon, hoverIcon, label, mnemonic, tooltip, style,
				null, false));
	}
	...
	private boolean shouldRestoreAppearance(final IHandler handler) {
	 ...
	 * <p>
	 * <strong>NOTE:</strong> The returned object should be treated as 'read-only', do <b>not</b> execute this instance or attempt to modify its state.
	 * </p>
	 *
	...
	void createCommand(final String commandId, final Map parameters) {
		if (commandId == null) {
			StatusManager.getManager().handle(StatusUtil.newStatus(IStatus.ERROR, "Unable to create menu item \"" + getId() //$NON-NLS-1$
	...
	/**
	 * Determines if the selection was on the dropdown affordance and, if so, opens the drop down menu (populated using the same id as this item...
	 *
	 * @param event
	...
	private void updateIcons() {
		if (widget instanceof MenuItem) {
			...
			try {
				...
				StatusManager.getManager().handle(new Status(IStatus.ERROR, WorkbenchPlugin.PI_WORKBENCH, "Failed to load image", e)); //$NON-NLS-1$
			}
			disposeOldImages();
	 ...
	 *
	 * @return a {@link CommandContributionItemParameter}. Valid fields are serviceLocator, id, style, icon, disabledIcon, hoverIcon, label, helpContextId,
	 *         mnemonic, tooltip. The Object will never be <code>null</code>, although any of the fields may be <code>null</code>.
	 * @since 3.100
	 */
