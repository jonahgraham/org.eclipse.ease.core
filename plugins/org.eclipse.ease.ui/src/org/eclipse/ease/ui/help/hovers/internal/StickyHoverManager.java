 ...
 * </p>
 * <p>
 * Clients usually instantiate and configure this class before using it. The configuration must be consistent: This means the used
 * {@link org.eclipse.jface.text.IInformationControlCreator} must create an information control expecting information in the same format the configured
...
public class StickyHoverManager extends InformationControlReplacer implements IWidgetTokenKeeper, IWidgetTokenKeeperExtension {
	...
	/**
	 * Internal information control closer. Listens to several events issued by its subject control and closes the information control when necessary.
	 */
		...
		public void handleEvent(Event event) {
			if (event.type == SWT.MouseMove) {
				...
				if ((infoControl != null) && !infoControl.isFocusControl() && (infoControl instanceof IInformationControlExtension3)) {
					...
					/*
					 * TODO: need better understanding of why/if this is needed. Looks like the same panic code we have in
					 * org.eclipse.jface.text.AbstractHoverInformationControlManager.Closer.handleMouseMove(Event)
					 */
					if ((fDisplay != null) && !fDisplay.isDisposed())
						fDisplay.removeFilter(SWT.MouseMove, this);
				}
	...
	private final IWidgetTokenOwner fWidgetTokenOwner;
	 ...
	 *            the text viewer
	 */
	public StickyHoverManager(Control hoverOrigin, Control hoverArea, IWidgetTokenOwner owner) {
