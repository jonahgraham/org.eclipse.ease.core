...
/**
 * An information control manager that shows information in response to mouse hover events. The mouse hover events are caught by registering a
 * {@link org.eclipse.swt.events.MouseTrackListener} on the manager's subject control. The manager has by default an information control closer that closes the
 * information control as soon as the mouse pointer leaves the subject area, the user presses a key, or the subject control is resized, moved, or deactivated.
 * <p>
 * When being activated by a mouse hover event, the manager disables itself, until the mouse leaves the subject area. Thus, the manager is usually still
...
abstract public class AbstractHoverInformationControlManager extends AbstractInformationControlManager {
	...
	/**
	 * The information control closer for the hover information. Closes the information control as soon as the mouse pointer leaves the subject area (unless
	 * "move into hover" is enabled), a mouse button is pressed, the user presses a key, or the subject control is resized, moved, or loses focus.
	 */
	protected class Closer implements IInformationControlCloser, MouseListener, MouseMoveListener, ControlListener, KeyListener, SelectionListener, Listener {

		/** The closer's subject control */
		private Control fSubjectControl;
		/** The subject area */
		protected Rectangle fSubjectArea;
		...
		public void start(Rectangle subjectArea) {
			...
			if ((fSubjectControl != null) && !fSubjectControl.isDisposed()) {
				...
				if (fSubjectControl instanceof Scrollable) {
					final Scrollable scrollable = (Scrollable) fSubjectControl;
					final ScrollBar vBar = scrollable.getVerticalBar();
					if (vBar != null)
						vBar.addSelectionListener(this);
					final ScrollBar hBar = scrollable.getHorizontalBar();
					if (hBar != null)
		...
		public void handleEvent(Event event) {
			...
			case SWT.MouseVerticalWheel:
				if (!hasInformationControlReplacer())
					...
					final IInformationControl infoControl = getCurrentInformationControl();
						...
						if (!(iControl5.containsControl(control))) {
							...
							if (event.type == SWT.MouseUp) {
								...
								if (infoControl instanceof IDelayedInputChangeProvider) {
									...
									final IInputChangedListener inputChangeListener = new DelayedInputChangeListener(delayedICP,
		...
		private void handleMouseMove(Event event) {
			...
			final Point mouseLoc = event.display.map(eventControl, fSubjectControl, event.x, event.y);
			...
			final IInformationControlExtension3 iControl3 = (IInformationControlExtension3) iControl;
			final Rectangle controlBounds = iControl3.getBounds();
			if (controlBounds != null) {
				final Rectangle tooltipBounds = event.display.map(null, eventControl, controlBounds);
			...
			if (!fSubjectControl.getBounds().contains(mouseLoc)) {
				/*
				 * Use inKeepUpZone() to make sure it also works when the hover is completely outside of the subject control.
				 */
	 ...
	 * <ul>
	 * <li>start function: initiates the computation of the information to be presented. This happens on receipt of a mouse hover event and disables the
	 * information control manager,
	 * <li>restart function: tracks mouse move and shell activation event to determine when the information control manager needs to be reactivated.
	 * </ul>
	 */
	class MouseTracker extends ShellAdapter implements MouseTrackListener, MouseMoveListener {
		...
		/**
		 * Sets this mouse tracker's subject area, the area to be tracked in order to re-enable the information control manager.
		 *
		 * @param subjectArea
		 *            the subject area
		 */
		...
		/**
		 * Starts this mouse tracker. The given control becomes this tracker's subject control. Installs itself as mouse track listener on the subject control.
		 *
		...
		/**
		 * Starts this mouse tracker. The given control becomes this tracker's subject control. Installs itself as mouse track listener on the subject control.
		 * The mouse move listener gets installed on the area control. The subject control is the element the hover element is attached to. The area control is
		 * the container that holds the subject control. It denotes the area to be tracked for mouse move events while the hover is open. Usually this should be
		 * the root composite of the current view. If the area is too small, the hover will not close correctly on some occasions.
		 *
		 * @param subjectControl
		 *            the subject control
		 * @param areaControl
		...
		/**
		 * Stops this mouse tracker. Removes itself as mouse track, mouse move, and shell listener from the subject control.
		 */
		public void stop() {
			if ((fSubjectControl != null) && !fSubjectControl.isDisposed()) {
		...
		/**
		 * Initiates the computation of the information to be presented. Sets the initial hover area to a small rectangle around the hover event location. Adds
		 * mouse move and shell activation listeners to track whether the computed information is, after completion, useful for presentation and to implement
		...
		public void mouseHover(MouseEvent event) {
			if (fIsComputing || fIsInRestartMode || ((fSubjectControl != null) && !fSubjectControl.isDisposed()
					&& (fSubjectControl.getShell() != fSubjectControl.getShell().getDisplay().getActiveShell()))) {
				if (DEBUG)
					System.out.println("AbstractHoverInformationControlManager...mouseHover: @ " + event.x + "/" + event.y //$NON-NLS-1$ //$NON-NLS-2$
							+ " : hover cancelled: fIsComputing= " + fIsComputing + ", fIsInRestartMode= " + fIsInRestartMode); //$NON-NLS-1$ //$NON-NLS-2$
				return;
			}
		...
		/**
		 * Deactivates this tracker's restart function and enables the information control manager. Does not have any effect if the tracker is still executing
		 * the start function (i.e. computing the information to be presented.
		 */
		...
		public void mouseExit(MouseEvent e) {
			if (!hasInformationControlReplacer() || !canMoveIntoInformationControl(getCurrentInformationControl())
					|| !inKeepUpZone(e.x, e.y, fSubjectControl, fSubjectArea, false)) {
		...
		/**
		 * Determines whether the computed information is still useful for presentation. This is not the case, if the shell of the subject control has been
		 * deactivated, the mouse left the subject control, or the mouse moved on, so that it is no longer in the subject area.
		 *
		...
		public boolean isMouseLost() {
			...
			if ((fSubjectControl != null) && !fSubjectControl.isDisposed()) {
				final Display display = fSubjectControl.getDisplay();
	...
	/**
	 * The delay in {@link ITextViewerExtension8.EnrichMode#AFTER_DELAY} mode after which the hover is enriched when the mouse has stopped moving inside the
	 * hover.
	 *
	 * @since 3.4
	 */
	...
	/**
	 * The thread that delays replacing of the hover information control. To be accessed in the UI thread only!
	 *
	 * @since 3.4
	 */
	...
	/**
	 * Indicates whether we have received a MouseDown event and are waiting for a MouseUp (and don't replace the information control until that happened).
	 *
	 * @since 3.4
	 */
	...
	/**
	 * Creates a new hover information control manager using the given information control creator. By default a <code>Closer</code> instance is set as this
	 * manager's closer.
	 *
	 * @param creator
	...
	/**
	 * Tests whether a given mouse location is within the keep-up zone. The hover should not be hidden as long as the mouse stays inside this zone.
	 *
	 * @param x
	 *            the x coordinate, relative to the <em>subject control</em>
	 * @param y
	 *            the y coordinate, relative to the <em>subject control</em>
	 * @param subjectControl
	 *            the subject control
	 * @param subjectArea
	 *            the area for which the presented information is valid
	 * @param blowUp
	 *            If <code>true</code>, then calculate for the closer, i.e. blow up the keepUp area. If <code>false</code>, then use tight bounds for hover
	 *            detection.
	 *
	...
	private boolean inKeepUpZone(int x, int y, Control subjectControl, Rectangle subjectArea, boolean blowUp) {
		...
		if (iControl instanceof IInformationControlExtension3) {
			...
			final Rectangle iControlBounds = subjectControl.getDisplay().map(null, subjectControl, iControl3.getBounds());
			final Rectangle totalBounds = Geometry.copy(iControlBounds);
			if (blowUp && isReplaceInProgress()) {
				...
				final int margin = getInformationControlReplacer().getKeepUpMargin();
	...
	/**
	 * Tests whether the given information control allows the mouse to be moved into it.
	 *
	 * @param iControl
	 *            information control or <code>null</code> if none
	 * @return <code>true</code> if information control allows mouse move into control, <code>false</code> otherwise
	 */
	...
	/**
	 * Sets the hover enrich mode. Only applicable when an information control replacer has been set with
	...
	boolean cancelReplacingDelay() {
		...
		if ((fReplacingDelayJob != null) && (fReplacingDelayJob.getState() != Job.RUNNING)) {
			final boolean cancelled = fReplacingDelayJob.cancel();
	...
	/**
	 * Starts replacing the information control, considering the current {@link ITextViewerExtension8.EnrichMode}. If set to
	 * {@link ITextViewerExtension8.EnrichMode#AFTER_DELAY}, this method cancels previous requests and restarts the delay timer.
	 *
	 * @param display
	 *            the display to be used for the call to {@link #replaceInformationControl(boolean)} in the UI thread
	 */
	...
	protected void presentInformation() {
		...
		final Rectangle area = getSubjectArea();
		if (area != null)
	...
	public void setEnabled(boolean enabled) {
		...
		final boolean was = isEnabled();
		super.setEnabled(enabled);
		final boolean is = isEnabled();
	...
	/**
	 * Installs this manager on the given subject control. The hover control is now taking the role of the subject control. This implementation sets the control
	 * also as the information control closer's subject control and automatically enables this manager. The area control typically is the root composite of the
	 * display element (e.g. a window or a dialog) the subject control is embedded in. It is needed to correctly close popups when the mouse pointer leaves the
	 * popup area.
	 *
	 * @param subjectControl
	 *            the subject control
