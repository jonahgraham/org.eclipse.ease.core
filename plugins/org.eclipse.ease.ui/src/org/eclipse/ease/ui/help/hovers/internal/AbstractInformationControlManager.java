...
/**
 * Manages the life cycle, visibility, layout, and contents of an {@link org.eclipse.jface.text.IInformationControl}. This manager can be installed on and
 * removed from a control, referred to as the subject control, i.e. the one from which the subject of the information to be shown is retrieved. Also a manager
 * can be enabled or disabled. An installed and enabled manager can be forced to show information in its information control using <code>showInformation</code>.
 * An information control manager uses an <code>IInformationControlCloser</code> to define the behavior when a presented information control must be closed. The
 * disposal of the subject and the information control are internally handled by the information control manager and are not the responsibility of the
...
abstract public class AbstractInformationControlManager {
	...
	/**
	 * Interface of an information control closer. An information control closer monitors its information control and its subject control and closes the
	...
	public interface IInformationControlCloser {
		...
		/**
		 * Sets the closer's subject control. This is the control that parents the information control and from which the subject of the information to be shown
		 * is retrieved.
		 * <p>
		 * Must be called before <code>start</code>. May again be called between <code>start</code> and <code>stop</code>.
		 *
		 * @param subject
		 *            the subject control
		 */
		...
		/**
		 * Tells this closer to start monitoring the subject and the information control. The presented information is considered valid for the given area of
		 * the subject control's display.
		 *
		 * @param subjectArea
	...
	public static final class Anchor {
		...
		/**
		 * Returns the SWT direction flag. One of {@link SWT#BOTTOM}, {@link SWT#TOP}, {@link SWT#LEFT}, {@link SWT#RIGHT}, {@link SWT#CENTER},
		 *
		...
		public String toString() {
			...
			default:
				return Integer.toHexString(fFlag);
			}
		}
	}
	 ...
	 *
	 * <p>
	 * This field should not be referenced by subclasses. It is <code>protected</code> for API compatibility reasons.
	 */
	...
	/**
	 * The information control replacer to be used when this information control needs to be replaced with another information control.
	 *
	 * @since 3.4
	 */
	...
	/**
	 * The anchor sequence used to layout the information control if the original anchor can not be used because the information control would not fit in the
	 * display client area.
	 * <p>
	 * The fallback anchor for a given anchor is the one that comes directly after the given anchor or is the first one in the sequence if the given anchor is
	...
	/**
	 * Tells whether the control's location should be read from the dialog settings and whether the last valid control's size is stored back into the settings.
	 *
	 * @since 3.0
	 */
	...
	/**
	 * Tells whether the control's size should be read from the dialog settings and whether the last valid control's size is stored back into the settings.
	 *
	 * @since 3.0
	 */
	...
	/**
	 * Computes the information to be displayed and the area in which the computed information is valid. Implementation of this method must finish their
	 * computation by setting the computation results using <code>setInformation</code>.
	 */
	abstract protected void computeInformation();

	/**
	 * Sets the parameters of the information to be displayed. These are the information itself and the area for which the given information is valid. This so
	 * called subject area is a graphical region of the information control's subject control. This method calls <code>presentInformation()</code> to trigger
	 * the presentation of the computed information.
	 *
	 * @param information
	 *            the information, or <code>null</code> if none is available
	 * @param subjectArea
	 *            the subject area, or <code>null</code> if none is available
	 */
	...
	/**
	 * Sets the information control replacer for this manager and disposes the old one if set.
	 *
	 * @param replacer
	 *            the information control replacer for this manager, or <code>null</code> if no information control replacing should take place
	 * @since 3.4
	 */
	 ...
	 *
	 * @param iControl
	 *            information control or <code>null</code> if none
	...
	boolean canReplace(IInformationControl iControl) {
		return (iControl instanceof IInformationControlExtension3) && (iControl instanceof IInformationControlExtension5)
	...
	/**
	 * Tells this information control manager to open the information control with the values contained in the given dialog settings and to store the control's
	 * last valid size in the given dialog settings.
	 * <p>
	 * Note: This API is only valid if the information control implements {@link IInformationControlExtension3}. Not following this restriction will later
	...
	/**
	 * Sets the anchor used for laying out the information control relative to the subject control. E.g, using <code>ANCHOR_TOP</code> indicates that the
	 * information control is position above the area for which the information to be displayed is valid.
	 *
	 * @param anchor
	 *            the layout anchor
	 */
	...
	/**
	 * Sets the anchors fallback sequence used to layout the information control if the original anchor can not be used because the information control would
	 * not fit in the display client area.
	 * <p>
	 * The fallback anchor for a given anchor is the one that comes directly after the given anchor or is the first one in the sequence if the given anchor is
	 * the last one in the sequence.
	 * <p>
	 ...
	 *            the creator, possibly <code>null</code>
	 * @since 3.0
	 */
	protected void setCustomInformationControlCreator(IInformationControlCreator informationControlCreator) {
		if ((informationControlCreator != null) && (fCustomInformationControlCreator instanceof IInformationControlCreatorExtension)) {
			final IInformationControlCreatorExtension extension = (IInformationControlCreatorExtension) fCustomInformationControlCreator;
	 ...
	 *
	 * @return <code>true</code> if the control takes focus when visible, <code>false</code> otherwise
	 * @since 3.7
	 */
	...
	/**
	 * Handles the disposal of the subject control. By default, the information control is disposed by calling <code>disposeInformationControl</code>.
	...
	/**
	 * Installs this manager on the given control. The control is now taking the role of the subject control. This implementation sets the control also as the
	 * information control closer's subject control and automatically enables this manager.
	 *
	...
	/**
	 * Computes the size constraints of the information control in points based on the default font of the given subject control as well as the size constraints
	 * in character width.
	 *
	 * @param subjectControl
	 *            the subject control
	...
	protected Point computeSizeConstraints(Control subjectControl, IInformationControl informationControl) {
		...
		if (fSizeConstraints == null) {
			if (informationControl instanceof IInformationControlExtension5) {
				final IInformationControlExtension5 iControl5 = (IInformationControlExtension5) informationControl;
			...
			final GC gc = new GC(subjectControl);
	...
	protected IInformationControl getInformationControl() {
		...
		if (fCustomInformationControlCreator == null) {
			...
			if (creator instanceof IInformationControlCreatorExtension) {
				final IInformationControlCreatorExtension extension = (IInformationControlCreatorExtension) creator;
	...
	/**
	 * Computes the display location of the information control. The location is computed considering the given subject area, the anchor at the subject area,
	 * and the size of the information control. This method does not care about whether the information control would be completely visible when placed at the
	 * result location.
	 *
	 * @param subjectArea
	 *            the subject area
	...
	protected Point computeLocation(Rectangle subjectArea, Point controlSize, Anchor anchor) {
		...
		case SWT.CENTER:
			final Point subjectControlSize = fSubjectControl.getSize();
			final Point location = new Point(subjectControlSize.x / 2, subjectControlSize.y / 2);
		...
		final boolean isRTL = (fSubjectControl != null) && ((fSubjectControl.getStyle() & SWT.RIGHT_TO_LEFT) != 0);
		if (isRTL)
	...
	/**
	 * Computes the area available for an information control given an anchor and the subject area within <code>bounds</code>.
	 *
	 * @param subjectArea
	 *            the subject area
	 * @param bounds
	 ...
	 * @param anchor
	 *            the anchor at the subject area
	 * @return the area available at the given anchor relative to the subject area, confined to the monitor's client area
	 * @since 3.3
	 */
	protected Rectangle computeAvailableArea(Rectangle subjectArea, Rectangle bounds, Anchor anchor) {
		...
		case SWT.CENTER:
			...
			break;
		case SWT.BOTTOM:
			final int y = subjectArea.y + subjectArea.height + fMarginY;
			area = new Rectangle(bounds.x, y, bounds.width, (bounds.y + bounds.height) - y);
			break;
		case SWT.RIGHT:
			final int x = subjectArea.x + subjectArea.width + fMarginX;
	...
	/**
	 * Checks whether a control of the given size at the given location would be completely visible in the given display area when laid out by using the given
	 * anchor. If not, this method tries to shift the control orthogonal to the direction given by the anchor to make it visible. If possible it updates the
	 * location.
	 * <p>
	 * This method returns <code>true</code> if the potentially updated position results in a completely visible control, or <code>false</code> otherwise.
	 *
	 *
	 * @param location
	...
	protected boolean updateLocation(Point location, Point size, Rectangle displayArea, Anchor anchor) {
		...
		final int displayLowerRightX = displayArea.x + displayArea.width;
		final int displayLowerRightY = displayArea.y + displayArea.height;
		final int lowerRightX = location.x + size.x;
		final int lowerRightY = location.y + size.y;
	 ...
	 * <p>
	 * The fallback anchor for the given anchor is the one that comes directly after the given anchor or is the first one in the sequence if the given anchor is
	 * the last one in the sequence.
	 * </p>
	 * <p>
	...
	/**
	 * Computes the location of the information control depending on the subject area and the size of the information control. This method attempts to find a
	 * location at which the information control lies completely in the display's client area while honoring the manager's default anchor. If this isn't
	 * possible using the default anchor, the fallback anchors are tried out.
	 *
	 * @param subjectArea
	...
	protected Point computeInformationControlLocation(Rectangle subjectArea, Point controlSize) {
		final Rectangle subjectAreaDisplayRelative = Geometry.toDisplay(fSubjectControl, subjectArea);
		...
		do {
			...
			final Monitor monitor = getClosestMonitor(subjectAreaDisplayRelative, testAnchor);
			...
			final Rectangle available = computeAvailableArea(subjectAreaDisplayRelative, monitor.getClientArea(), testAnchor);
			final Rectangle proposed = new Rectangle(upperLeft.x, upperLeft.y, controlSize.x, controlSize.y);
			available.intersect(proposed);
			final int area = available.width * available.height;
			if (area > bestArea) {
				bestArea = area;
	 ...
	 * @param anchor
	 *            the anchor
	 * @return the monitor closest to the edge of <code>area</code> defined by <code>anchor</code>
	 * @since 3.3
	 */
	...
	/**
	 * Copied from org.eclipse.jface.window.Window. Returns the monitor whose client area contains the given point. If no monitor contains the point, returns
	 * the monitor that is closest to the point. If this is ever made public, it should be moved into a separate utility class.
	 *
	 * @param display
	 *            the display to search for monitors
	 * @param rectangle
	 *            the rectangle to find the closest monitor for (display coordinates)
	...
	/**
	 * Computes information to be displayed as well as the subject area and initiates that this information is presented in the information control. This
	 * happens only if this controller is enabled.
	 */
	...
	/**
	 * Presents the information in the information control or hides the information control if no information should be presented. The information has
	 * previously been set using <code>setInformation</code>.
	 * <p>
	...
	private void internalShowInformationControl(Rectangle subjectArea, Object information) {
		...
		if (informationControl != null) {
			...
			final Point sizeConstraints = computeSizeConstraints(fSubjectControl, fSubjectArea, informationControl);
			...
			if (informationControl instanceof IInformationControlExtension) {
				final IInformationControlExtension extension = (IInformationControlExtension) informationControl;
			...
			Point size = null;
			Point location = null;
			final Rectangle bounds = restoreInformationControlBounds();
			...
			final Rectangle controlBounds = Geometry.createRectangle(location, size);
	...
	void cropToClosestMonitor(Rectangle bounds) {
		final Rectangle monitorBounds = getClosestMonitor(fSubjectControl.getDisplay(), bounds).getClientArea();
	...
	/**
	 * Replaces this manager's information control as defined by the information control replacer. <strong>Must only be called when {@link #fInformationControl}
	 * instanceof {@link IInformationControlExtension3}!</strong>
	 *
	 * @param takeFocus
	...
	void replaceInformationControl(boolean takeFocus) {
		if ((fInformationControlReplacer != null) && canReplace(fInformationControl)) {
			final IInformationControlExtension3 iControl3 = (IInformationControlExtension3) fInformationControl;
			final Rectangle b = iControl3.getBounds();
			final Rectangle t = iControl3.computeTrim();
			final Rectangle contentBounds = new Rectangle(b.x - t.x, b.y - t.y, b.width - t.width, b.height - t.height);
			final IInformationControlCreator informationPresenterControlCreator = ((IInformationControlExtension5) fInformationControl)
	...
	protected void storeInformationControlBounds() {
		...
		final boolean controlRestoresSize = ((IInformationControlExtension3) fInformationControl).restoresSize();
		final boolean controlRestoresLocation = ((IInformationControlExtension3) fInformationControl).restoresLocation();

		final Rectangle bounds = ((IInformationControlExtension3) fInformationControl).getBounds();
		if (bounds == null)
			return;
	...
	protected Rectangle restoreInformationControlBounds() {
		...
		final Rectangle bounds = new Rectangle(-1, -1, -1, -1);
